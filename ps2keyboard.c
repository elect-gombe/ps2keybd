// PS/2キーボード読み込みプログラム for PIC32MX by K.Tanaka　Rev1.4　2016.5.2
...
// PIC32MX370F512H版 DAT:RD6、CLK:RD7
//
...
void __ISR(33, ipl6) CNHandler(void)
{
	...
	if(cnstatd){
		...
		mCNDClearIntFlag(); // CND割り込みフラグクリア
	}
}
#else
...
#ifdef PIC32MX370F
			dummy=PORTD;//割り込み無効化中のポート変化を吸収
			CNENDSET=PS2CLKBIT; // CLKの変化でCN割り込み有効
#else
			dummy=PORTB;//割り込み無効化中のポート変化を吸収
			CNENBSET=PS2CLKBIT; // CLKの変化でCN割り込み有効
#endif
